...
strawb_survey <- strawb_survey %>%
  mutate(`Chemical Name` = str_extract(Domain.Category, "\\((.*?)\\)")) %>%  # Extract text inside parentheses
  mutate(`Chemical Name` = str_remove_all(`Chemical Name`, "[()]")) %>%  # Remove parentheses
  mutate(`Chemical Name` = str_remove(`Chemical Name`, " = \\d+$")) %>%  # Remove "= number" at the end
